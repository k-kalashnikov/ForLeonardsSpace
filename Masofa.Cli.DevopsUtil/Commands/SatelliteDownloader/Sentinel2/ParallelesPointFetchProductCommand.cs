...
{
    [BaseCommand("Paralleles Fetch Points Products And Indices Command", "Fetch Products And Indices Command")]
    public class ParallelesPointFetchProductCommand : IBaseCommand
    {
        ILogger<ParallelesPointFetchProductCommand> Logger { get; }
        ...
        public ParallelesPointFetchProductCommand(ILogger<ParallelesPointFetchProductCommand> logger,
        ...
        public async Task Execute()
        {
            try
            {
                ...
                var needProductsStates = SentinelDbContext.Sentinel2GenerateIndexStatus.ToList()
                    .Where(ps => !ps.IsDbComplite).ToList();
                ...
                await GeneratePoints(satelliteProductsWithFields, needProducts, needProductsQueue, needProductsStates, 3);
        ...
        private async Task FetchStatus()
        {
            ...
            Console.WriteLine("Fetching OriginDate");
            var index = 0;
        ...
        private Dictionary<Guid, List<Masofa.Common.Models.CropMonitoring.Field>> GetSatelluteProductWithFields(List<Masofa.Common.Models.Satellite.SatelliteProduct> sourceProduct)
        {
            ...
            try
            {
                ...
                var fields = MasofaCropMonitoringDbContext.Fields
                    .Where(m => m.Id == Guid.Parse("5bd6a452-d14c-4883-9b0d-e8d7e8e1a4ac"))
                    .ToList();
        ...
        #region Steps
        private async Task<List<Sentinel2ProductQueue>> GeneratePoints(Dictionary<Guid, List<Masofa.Common.Models.CropMonitoring.Field>> satelliteProductsWithFields, List<SatelliteProduct> needProducts,
            ...
            List<Sentinel2GenerateIndexStatus> productQueueStatuses, int outputLine)
        {
            ...
            foreach (var productWithField in satelliteProductsWithFields)
            {
                ...
                DrawProgressBar(outputLine, index, satelliteProductsWithFields.Count, "GenerateTiffProducts", $"Start for {index} of {satelliteProductsWithFields.Count} product");
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        DrawProgressBar(outputLine + index, index, satelliteProductsWithFields.Count, "GenerateTiffProducts", $"Complite for {needProductQueueItem.ProductId} with " +
                    ...
                    {
                        DrawProgressBar(outputLine + index, index, satelliteProductsWithFields.Count, "GenerateTiffProducts", $"Exception: {ex.Message}");
                    }
                ...
                {
                    DrawProgressBar(outputLine, index, satelliteProductsWithFields.Count, "GenerateTiffProducts", $"Exception: {ex.Message}");
                    continue;
                }
                finally
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(tempExtractPath) && Directory.Exists(tempExtractPath))
                        {
                            ...
                            DrawProgressBar(outputLine, index, satelliteProductsWithFields.Count, "GenerateTiffProducts", $"Cleaned up temp extract path: {tempExtractPath}");
                        }
                    }
                    catch (Exception cleanupEx)
                    {
                        DrawProgressBar(outputLine, index, satelliteProductsWithFields.Count, "GenerateTiffProducts", $"Cleanup error: {cleanupEx.Message}");
                    }
                }
            }

            DrawProgressBar(outputLine, satelliteProductsWithFields.Count, satelliteProductsWithFields.Count, "GenerateTiffProducts", "ARVIJob completed successfully.");
            return result;
        }
        private Dictionary<string, TiffGenerationResult> GenerateOneProductPoint(Sentinel2ProductQueue needProduct, Sentinel2GenerateIndexStatus productStatus, string archiveFolder)
        {
