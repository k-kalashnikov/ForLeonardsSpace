...
//{
//    public class MigrateWeatherExportSqlCommandParameters
//    {
//        [TaskParameter("Базовый путь для экспорта", true, "C:\\Export")]
//        public string BaseExportPath { get; set; } = string.Empty;

//        public static MigrateWeatherExportSqlCommandParameters Parse(string[] args)
//        {
//            if (args.Length == 0 || string.IsNullOrEmpty(args[0]))
//                throw new ArgumentException("Необходимо указать базовый путь для экспорта");

//            return new MigrateWeatherExportSqlCommandParameters { BaseExportPath = args[0] };
//        }

//        public static MigrateWeatherExportSqlCommandParameters GetFromUser()
//        {
//            Console.WriteLine("Enter base export path:");
//            var baseExportPath = Console.ReadLine() ?? string.Empty;
//            return new MigrateWeatherExportSqlCommandParameters { BaseExportPath = baseExportPath };
//        }
//    }

//    [BaseCommand("Migrate Weather Export Sql", "Миграция погодных данных с экспортом в SQL", typeof(MigrateWeatherExportSqlCommandParameters))]
//    public class MigrateWeatherExportSqlCommand : IBaseCommand
//    {
...
//        public MigrateWeatherExportSqlCommand(MasofaWeatherDbContext masofaWeatherDbContext, DepricatedWeatherServerOneDbContext depricatedWeatherServerOneDbContext, MasofaDictionariesDbContext masofaDictionariesDbContext)
//        {
...
//        {
//            var parameters = MigrateWeatherExportSqlCommandParameters.Parse(args);
//            await ExecuteCore(parameters);
//        }

//        public async Task Execute()
//        {
//            var parameters = MigrateWeatherExportSqlCommandParameters.GetFromUser();
//            await ExecuteCore(parameters);
//        }

//        private async Task ExecuteCore(MigrateWeatherExportSqlCommandParameters parameters)
//        {
//            Console.WriteLine($"Base export path: {parameters.BaseExportPath}");
//            await ImportWeatherTablesAsync(parameters.BaseExportPath);
//        }
...
//        private async Task ImportWeatherTablesAsync(string baseExportPath)
//        {
...
//                            var fileName = Path.Combine(baseExportPath, nameof(Masofa.Common.Models.Weather.WeatherStationsDatum).ToString(), $"{folderIterator.ToString()}" ,$"{(currentPage - (folderIterator * 1000)).ToString()}.sql");
//                            var direrotyPath = Path.Combine(baseExportPath, nameof(Masofa.Common.Models.Weather.WeatherStationsDatum).ToString(), $"{folderIterator.ToString()}");
//                            if (!Directory.Exists(direrotyPath))
//                            {
//                                EnsureDirectoriesExistRecursive(direrotyPath);
//                            }
//                            File.AppendAllLines(fileName, sqlStrs.Select(m => m.Value.Item1).ToList());
//                            Console.WriteLine($"Insert script in to {nameof(Masofa.Common.Models.Weather.WeatherStationsDatum).ToString()} saved in {fileName}");
//                        }
//                        else
//                        {
...
//                        currentPage++;
//                        folderIterator = (currentPage / 1000);
//                    }
...
//        private string ResolveToSqlString(PropertyInfo prop, object item)
//        {
