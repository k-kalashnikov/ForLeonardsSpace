...
{
    [BaseCommand("Create weather tiles for Normalized Era5 data", "Create weather tiles for Normalized Era5 data")]
    public class EraNormalizedDataTilesCommand : IBaseCommand
    {
        ...
        private readonly string type = "Normalized";
        ...
        public EraNormalizedDataTilesCommand(
            ...
            MasofaTileDbContext masofaTileDbContext)
        {
            ...
            _outputPath = configuration.GetValue<string>("Era5:TilesFolder") ?? "/deploy/prod/data-geoserver-prod";
        ...
        public void Dispose()
        {
            Console.WriteLine($"EraNormalizedDataTilesCommand end");
        }
        ...
        public async Task Execute()
        {
            ...
            foreach (var month in normalizedData.Keys)
            {
                foreach (var day in normalizedData[month].Keys)
                {
                    foreach (var prop in valueProperties)
                    {
                        try
                        {
                            await CreateReportTiff(month, day, prop, width, height, gt, [.. normalizedData[month][day].Cast<BaseEra5WeatherReport>()], stations);
                        }
                        catch (Exception ex)
                        {
            ...
            Console.WriteLine($"EraNormalizedDataTilesCommand start");
        }
        ...
        private async Task CreateReportTiff(int month, int day, PropertyInfo prop, int width, int height, double[] gt, List<BaseEra5WeatherReport>? reports, Dictionary<Guid, NetTopologySuite.Geometries.Point>? stations)
        {
            #region CreateEmptyTiff
            var fieldName = $"Normalized{prop.Name}";
            var options = new[] {
                    "COMPRESS=LZW",
                    "TILED=YES",
                    "ALPHA=YES"
            ...
            var formatedDate = $"{month:D2}{day:D2}";
