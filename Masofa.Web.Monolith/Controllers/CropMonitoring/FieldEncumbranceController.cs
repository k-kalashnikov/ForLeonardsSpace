...
{
    /// <summary>
    /// Provides API endpoints for managing field encumbrance records (обременения).
    /// </summary>
    /// <remarks>This controller supports CRUD operations for field encumbrances, leveraging the
    ...
    public class FieldEncumbranceController : BaseCrudController<Masofa.Common.Models.CropMonitoring.FieldEncumbrance, Masofa.DataAccess.MasofaCropMonitoringDbContext>
    {
        public FieldEncumbranceController(IFileStorageProvider fileStorageProvider, MasofaCropMonitoringDbContext dbContext, ILogger<FieldEncumbranceController> logger, IConfiguration configuration, IMediator mediator, IBusinessLogicLogger businessLogicLogger, IHttpContextAccessor httpContextAccessor) : base(fileStorageProvider, dbContext, logger, configuration, mediator, businessLogicLogger, httpContextAccessor)
        {
