...
{
    [SystemBackgroundTask(BackgroundTaskType.NotCritical, "UgmForecastWeatherReportsTilesGenerationJob", "Ugm")]
    public class UgmForecastWeatherReportsTilesGenerationJob : BaseJob<UgmForecastWeatherReportsTilesGenerationJobResult>, IJob
    {
        ...
        public UgmForecastWeatherReportsTilesGenerationJob(
            IMediator mediator,
            IBusinessLogicLogger businessLogicLogger,
            ILogger<UgmForecastWeatherReportsTilesGenerationJob> logger,
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                var forecastData = await UgmDbContext.UgmWeatherData
                    .Where(d => d.Date != null && d.Date > dateToday && d.DayPart == DayPart.Day)
                    .GroupBy(d => d.Date.Value)
                    .ToDictionaryAsync(
                        d => d.Key,
                        d => d.ToDictionary(item => item.RegionId,
                                            item => item)
                    );

                HashSet<int> regionIds = [];
                foreach (var kv in forecastData.Values)
                {
                    regionIds.UnionWith(kv.Keys);
                }

                var stations = await UgmDbContext.UgmWeatherStations
                    .Where(s => regionIds.Contains(s.UgmRegionId))
                    .ToListAsync();
                ...
                foreach (var dataByDate in forecastData)
                {
                    var data = dataByDate.Value;
                    var drv = Gdal.GetDriverByName("GTiff");
                    var formatedDate = $"{dataByDate.Key:yyyyMMdd}";
    ...
    public class UgmForecastWeatherReportsTilesGenerationJobResult : BaseJobResult
    {
