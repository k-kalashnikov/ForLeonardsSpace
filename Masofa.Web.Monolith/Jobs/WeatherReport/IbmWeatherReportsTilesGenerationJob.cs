...
{
    [SystemBackgroundTask(BackgroundTaskType.NotCritical, "IbmWeatherReportsTilesGenerationJob", "IBMWeather")]
    public class IbmWeatherReportsTilesGenerationJob : BaseJob<IbmWeatherReportsTilesGenerationJobResult>, IJob
    {
        ...
        private readonly string source = "Ibm";
        private readonly string type = "Weather";
        private readonly string _outputPath;
        private readonly string _workspace;
        private double lon0, lat0, lon1, lat1, ibmPixel, finePixel;
        private int width, height, superFactor;
        private Geometry? enlargedPolygon = null;
        private MultiPolygon? countryMp = null;

        public IbmWeatherReportsTilesGenerationJob(
            IMediator mediator,
            IBusinessLogicLogger businessLogicLogger,
            ILogger<IbmWeatherReportsTilesGenerationJob> logger,
            MasofaIBMWeatherDbContext eraDbContext,
            ...
            MasofaIdentityDbContext identityDbContext) : base(mediator, businessLogicLogger, logger, commonDbContext, identityDbContext)
        {
            ...
            ibmPixel = configuration.GetValue<double>("IBMWeather:Step");
            GdalInitializer = gdalInitializer;
        }
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                var forecastsByDate = await IbmDbContext.IbmDayWeatherForecasts
                ...
                var stationIds = forecastStationIds.Distinct().ToList();

                var stations = await IbmDbContext.IBMMeteoStations
                ...
                superFactor = (int)Math.Round(ibmPixel / finePixel);
                ...
                foreach (var (forecastDate, forecasts) in forecastsByDate)
                {
                    foreach (var prop in valueProperties)
                    {
                        try
                        {
                            await CreateReportTiff(forecastDate, prop, width, height, gt, [.. forecasts.Cast<BaseIbmWeatherReport>()], stations);
                        }
                        catch (Exception ex)
                        {
        ...
        private async Task CreateFrostDangerTiff(DateOnly reportDate, int width, int height, double[] gt, List<IbmDayWeatherForecast>? reports, Dictionary<Guid, NetTopologySuite.Geometries.Point>? stations)
        {
            ...
            foreach (var report in reports)
            {
                if (stations.TryGetValue(report.WeatherStation.Value, out var stationPoint))
                {
                    ...
                    double cellW = lon - ibmPixel / 2.0;   // left
                    double cellE = lon + ibmPixel / 2.0;   // right
                    double cellS = lat - ibmPixel / 2.0;   // bottom
                    double cellN = lat + ibmPixel / 2.0;   // top
            ...
            ds.SetMetadataItem("PIXEL_SIZE", ibmPixel.ToString("F2"), "");
        ...
        private async Task CreateReportTiff(DateOnly reportDate, PropertyInfo prop, int width, int height, double[] gt, List<BaseIbmWeatherReport>? reports, Dictionary<Guid, NetTopologySuite.Geometries.Point>? stations)
        {
            #region CreateEmptyTiff
            var options = new[] {
                    "COMPRESS=LZW",
    ...
    public class IbmWeatherReportsTilesGenerationJobResult : BaseJobResult
    {
